...
class Team(models.Model):
    city = models.CharField(
        ...
        default=CityChoices.ATLANTA
    )
    abbr = models.CharField(
        max_length=3,
        choices=TeamChoices.choices,
        default=TeamChoices.CARDINALS
    )
    stadium = models.CharField(
        max_length=4,
        choices=StadiumChoices.choices,
        default=StadiumChoices.AS
    )
    ...
    def __str__(self) -> str:
        return f"Team: {TeamChoices(self.abbr).label} from {CityChoices(self.city).label}"
...
class Game(models.Model):
    week = models.ForeignKey(
        Week, on_delete=models.CASCADE, related_name='games'
    )
    ...
    def __str__(self) -> str:
        return f"Game: {self.visitor_team} at {self.home_team} on {self.timestamp}"
...
class Pick(models.Model):
    ...
    game = models.ForeignKey(
        Game, on_delete=models.CASCADE, related_name='picks'
    )
    selection = models.SmallIntegerField(
        choices=PickChoices.choices,
        default=PickChoices.TBP
    )
    ...
    def __str__(self) -> str:
        return f"Pick: {self.user.first_name} - {self.game} - {self.selection}"
